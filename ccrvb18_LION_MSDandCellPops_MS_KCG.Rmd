...
params:  
  ...
  ### first time no spin
  includePreSpin: FALSE #first time true, then false to be replaced with spun
  evalSpun: TRUE #first time false, then true
...
includePreSpin <- params$includePreSpin
evalSpun <- params$evalSpun
```
...
do.call("grid.arrange",c(list_of_plots_mean, ncol=num_columns_for_plot))
  ...
  cat("\n")
  cat(' \n \n') ### this is the key!
    list_of_plotted[[contrast_name]] <- data_to_use
```
...
list_of_plotted[[paste0(contrast_name)]] <- data_to_use
```
...
if(plot_sig_only == "no"){
 ...
 list_of_plotted[[paste0(contrast_name)]] <- phen_to_plot
...
p
 ...
 list_of_plotted[[paste0(contrast_name)]] <- df_for_plot %>% 
...
do.call("grid.arrange",c(list_of_plots, ncol=num_columns_for_plot))
  ...
  list_of_plotted[[paste0(contrast_name)]] <- data_to_use %>%
    dplyr::select(c(Fig,MacaqueID,TimePoint,value_raw,log10_value)) 
...
do.call("grid.arrange",c(grobs = list_of_plots_ordered, ncol=num_columns_for_plot))
  ...
  list_of_plotted[[paste0(curr_figure_panel)]] <- data_to_use
...
##### set prior for log2 (0 for MSD, 1e-4 for cell pops, 0 for S1)
prior_to_use <- 0
curr_figure_name <- "FigS1"

##### select data
data_to_use <- list_of_datasets[["Figure S1"]] %>%
  ### add sampleName
  dplyr::mutate(sampleName = paste0("F",MacaqueID,"__",Day),
                Fig_Cell = paste0(Fig, "__",CellType),
                Fig = NULL,
                CellType = NULL) %>%
  ### make wider
  tidyr::pivot_wider(names_from = "Fig_Cell", values_from = "value_raw")
...
curr_df_long <- data_to_use %>%
  ...
  dplyr::mutate(log2_value = raw_value, ### no log transform of this data
                raw_value = NULL,
...
### select comparisons to make: time1 - time2
TimePoints_to_compare <- data.frame(
  "time1"=c("D2","D3","D4","D8","D15"),
  "time2"=c("D1","D1","D1","D1","D1")) %>%
  dplyr::mutate(name = paste0(time1,"-",time2))

timepoint_order <- c("D1","D2","D3","D4","D8","D15")
...
```{r differential_expression_FigS1, ref.label=c('differential_expression')}
```

### Fig S1A

```{r setVars_FigS1A}
curr_figure_panel <- "FigS1A"
varcomp_to_plot <- unique(curr_df_long$variable)[grepl(curr_figure_panel, unique(curr_df_long$variable))]
num_columns_for_plot <- 4
```

```{r Lineplots_with_pvalues_FigS1A, ref.label=c('Lineplots_with_pvalues'), eval=TRUE, results='asis', fig.height=6, fig.width=10}
```

### Fig S1B

```{r setVars_FigS1B}
curr_figure_panel <- "FigS1B"
varcomp_to_plot <- unique(curr_df_long$variable)[grepl(curr_figure_panel, unique(curr_df_long$variable))]
num_columns_for_plot <- 3
```

```{r Lineplots_with_pvalues_FigS1B, ref.label=c('Lineplots_with_pvalues'), eval=TRUE, results='asis', fig.height=6, fig.width=8}
```
...
```{r setVars_FigS1C}
curr_figure_panel <- "FigS1C"
varcomp_to_plot <- unique(curr_df_long$variable)[grepl(curr_figure_panel, unique(curr_df_long$variable))]
num_columns_for_plot <- 1
```

```{r Lineplots_with_pvalues_FigS1C, ref.label=c('Lineplots_with_pvalues'), eval=TRUE, results='asis', fig.height=3, fig.width=3}
```
  ...
  for (npt in 1:dim(TimePoints_to_compare)[1]){
    ...
    curr_vars <- colnames(curr_df2)[-which(colnames(curr_df2) %in% c("MacaqueID","TimePoint","TimePoint_f","ID_f"))]
      ...
      if(length(nonNA_columns)>0){
       ...
       ##### no scaling 
                curr_p_raw <- apply(new_variable_mat, 2, function(x) coef(summary(geeglm(x ~ TimePoint_f, id=ID_f, data = curr_df2, family="gaussian", corstr="exchangeable")))[2,'Pr(>|W|)'])
        curr_p_adj <- p.adjust(curr_p_raw, method = 'BH')
        curr_est <- apply(new_variable_mat, 2, function(x) coef(summary(geeglm(x ~ TimePoint_f, id=ID_f, data = curr_df2, family="gaussian", corstr="exchangeable")))[2,'Estimate'])
...
#### to collect plotted values
plotted_prep <- data.frame()
...
p <- ggplot(res.sub,aes(.data[[change_lfc_name]],-log10(.data[[change_sig_name]]),color=Sig_f, fill=Sig_f, label=.data[[label.col]])) +
  ...
  plotted_prep <- rbind(plotted_prep,res.sub %>% 
                          dplyr::mutate(TimePoint = paste0(name_simp_c))) 
...
cat(' \n \n') ### this is the key!
       ...
       list_of_plotted[[paste0(contrast_name)]] <- plotted_prep %>%
         dplyr::mutate(neg_log10p = -log10(!!rlang::sym(change_sig_name)))
```
...
cat(' \n \n') ### this is the key!
 ...
 list_of_plotted[[paste0(curr_figure_panel)]] <- degs_df_for_hm[rownames(curr_ann_row),rownames(annotation_col)] %>%
...
list_of_plotted[[paste0(contrast_name,"_log2FC")]] <- diff_counts %>%
...
#### plotted is same as stats, but filtered to p < 0.05
list_of_plotted[[paste0(contrast_name,"_spCorr")]] <- list_of_stats[[paste0(contrast_name)]] %>%
  dplyr::filter(p.value < p_val_levels[i])
```
...
### To name output files
contrast_name <- "FigS2A_MSD"
```


```{r differential_expression_FigS2A, ref.label=c('differential_expression')}
```



```{r FigS2A_Log2FCest_heatmaps_setup}
curr_figure_panel <- "FigS2A"
### read in degs results
degs_df <- list_of_stats[["FigS2A_MSD"]]
...
```{r Log2FCest_heatmaps_prep_FigS2A, include = includePreSpin}
mycols = diverging_hcl(n=100, palette="Blue-Red 3")
scale_range = 3
breakscale <- seq(-1*scale_range, scale_range, length=100)
...
```{r Log2FCest_heatmapSPUN_FigS2A, results = 'asis', fig.height=7, fig.width=6, eval=evalSpun}
...
list_of_plotted[[paste0(curr_figure_panel)]] <- degs_df_for_hm[,rownames(annotation_col)] %>%
  rownames_to_column("analyte")
```
...
```{r lineTraces_with_meanOrmedian__FigS2B, results='asis',fig.height=12, fig.width=6}


list_of_plots_mean <- list()
list_of_plots_median <- list()
plotted_prep <- data.frame() ### to save plotted values
## for each analyte of interest
for(i in selected_analytes){
 phen_to_plot <- df_long %>%
  ...
  ### save for plotted values
  plotted_prep <- rbind(plotted_prep,phen_to_plot)
...
do.call("grid.arrange",c(list_of_plots_median, ncol=num_columns_for_plot))
  ...
  cat(' \n \n') ### this is the key!
   ...
   list_of_plotted[[paste0(contrast_name)]] <- plotted_prep %>%
     ### add means
     dplyr::group_by(vax_dose_day,variable) %>%
     dplyr::mutate(mean = mean(raw_value),
                   median = median(raw_value),
                   vax_dose_day2 = NULL) %>%
     dplyr::relocate(variable,MacaqueID,vax_dose_day)
```
...
```{r alluvial_UPdownChangers_FigS2C, results='asis', fig.width=24, fig.height=15}

### stats are the same as for Fig3B_MSD_V4
list_of_stats[["FigS2C_MSD_V4"]] <- list_of_stats[["Fig3B_MSD_V4"]]

degs_df <- list_of_stats[["FigS2C_MSD_V4"]]
...
combo_df_long <- annotation_row %>%
 ...
 ####### plot
  ...
  cat(' \n \n') ### this is the key!
     ...
     list_of_plotted[[paste0("FigS2C_MSD_V4")]] <- combo_df_long
...
### To name output files
contrast_name <- "FigS3B_MSD"
```


```{r differential_expression_FigS3B, ref.label=c('differential_expression')}
```



```{r FigS3B_Log2FCest_heatmaps_setup}
curr_figure_panel <- "FigS3B"
### read in degs results
degs_df <- list_of_stats[["FigS3B_MSD"]]
...
```{r Log2FCest_heatmaps_prep_FigS3B, ref.label=c('Log2FCest_heatmaps_prep_FigS2A'),include = includePreSpin}
```

```{r Log2FCest_heatmapSPUN_FigS3B, ref.label=c('Log2FCest_heatmapSPUN_FigS2A'), results = 'asis', fig.height=7, fig.width=6, eval = evalSpun}
```
...
### To name output files
contrast_name <- "FigS3C_MSD"

num_columns_for_plot <- 4
```


```{r lineTraces_2groups__FigS3C, results='asis',fig.height=12, fig.width=12}
...
do.call("grid.arrange",c(list_of_plots, ncol=num_columns_for_plot))
  ...
  list_of_plotted[[paste0(contrast_name)]] <- df_long %>%
    dplyr::select(-vax_dose_day2) %>%
    ## make wider for ease of viewing
...
do.call("grid.arrange",c(list_of_plots, ncol=num_columns_for_plot))
  ...
  list_of_plotted[[paste0(curr_figure_panel)]] <- df_long %>%
    ### make wider 
    dplyr::select(-variable,-compartment) %>%
    tidyr::pivot_wider(names_from = "comp_var",values_from = "raw_value")
...
cDC1, cDC2 frequency of live cells

```{r prep_FigS4A}

##### set prior for log2 (0 for MSD, 1e-4 for cell pops)
prior_to_use <- 1e-4
curr_figure_name <- "FigS4A"
...
### To name output files
contrast_name <- "FigS4A"
curr_figure_panel <- "FigS4A"
...
### variables ordered in plots
variables_ordered <- unique(curr_df_long$variable)
```

```{r differential_expression_FigS4A, ref.label=c('differential_expression')}
```

```{r scatterplots_with_pvalues___FigS4A, ref.label=c('scatterplots_with_pvalues'), results='asis', fig.height=8, fig.width=8}
```
...
### To name output files
contrast_name <- "FigS4B"
curr_figure_panel <- "FigS4B"
...
```{r differential_expression_FigS4B, ref.label=c('differential_expression')}
```

```{r scatterplots_with_pvalues___FigS4B, ref.label=c('scatterplots_with_pvalues'), results='asis', fig.height=5, fig.width=5}
```
...
timepoint_order <- c("D1","D3","D8","D22")
...
df_long <- data_to_use %>%
  ...
  dplyr::mutate(comp_var = variable,
                ...
                Day = factor(str_after_first(sampleName,"__"),timepoint_order),
                MacaqueID = str_before_first(sampleName,"__"),
                sampleName = NULL) %>%
  dplyr::arrange(compartment, variable)
...
```{r Fig5C_hm}
curr_figure_panel <- "Fig5C_heatmap"
```
...
curr_ds <- list_of_plotted[[paste0(contrast_name,"_log2FC")]] %>%
  column_to_rownames("variable")

###### MSD = D2-D1 only log2FC
curr_df2 <- data.frame(t(list_of_plotted[[paste0("Fig3C_MSD_V4_D2vsD1_log2FC")]])) %>%
  ### make row 1 colnames
...
assay_arrangement <- c("CCL19.MIP.3b","CCL24.Eotaxin.2","CCL26.Eotaxin.3","CCL27.CTACK","CXCL13",
                       "FLT3L","CXCL12.SDF.1a","CCL11.Eotaxin","CCL22.MDC",
...
bub_df2$celltype <- cleaned_celltypes
  ...
  list_of_plotted[[paste0(curr_figure_panel)]] <- bub_df_bothordered %>%
    dplyr::select(-c(celltype,celltype_assay,comp_celltype)) %>%
    dplyr::relocate(compartment,celltype_only) %>%
    dplyr::rename(celltype=celltype_only)
...
barbara_order <- c("LN__Pct_of_HLADRpos_HLA_DRpos_Ki67pos",
                   "LN__Pct_of_HLADRpos_Classical_Mo_Ki67pos",
                   "LN__Pct_of_HLADRpos_Intermediate_Mo_Ki67pos",
                   "LN__Pct_of_HLADRpos_Non_classical_Mo_Ki67pos",
...
trim_pval_level <- 0.05
all_or_selected <- "selected"
assay_arrangement <- c("FLT3L","CCL13.MCP.4","CCL22.MDC","CXCL12.SDF.1a",
                       "G.CSF","IFN.g","CCL20.MIP.3a") ### if trimming/ordering assays


### set pos/neg/both
directions_to_use <- c("pos")
assays_to_exclude <- c("")
curr_figure_panel <- "Fig5D_LN"
```
...
for(i in 1:nrow(selected_pairs_df)){
 ...
 phen_to_plot <- df_long %>%
  ...
  # plot(p)
  list_of_plots[[i]] <- p
  plotted_prep <- rbind(plotted_prep, 
                        phen_to_plot %>%
                          dplyr::mutate(x_variable = paste0(curr_x,"___",curr_Day_x),
                                        y_variable = paste0(curr_y,"___",curr_Day_y)) %>%
                          dplyr::rename(x_value = !!rlang::sym(paste0(curr_x,"___",curr_Day_x)),
                                        y_value = !!rlang::sym(paste0(curr_y,"___",curr_Day_y))) %>%
                          dplyr::relocate(MacaqueID, x_variable, x_value, y_variable, y_value)
                        )
...
do.call("grid.arrange",c(list_of_plots, ncol=num_columns_for_plot))
  ...
  list_of_plotted[[paste0(curr_figure_panel)]] <- plotted_prep 
...
list_of_stats[["FigS5A"]] <- data.frame("FigS5A"="stats are in the Fig5ABC tab")
curr_figure_panel <- "FigS5A"
num_columns_for_plot <- 2
```

```{r prep_FigS5A, ref.label=c('prep_Fig5')}
```

```{r Lineplots_with_pvalues_FigS5A, ref.label=c('Lineplots_with_pvalues'), eval=TRUE, results='asis', fig.height=6, fig.width=6}
```
...
curr_phen_long  <- data_to_use %>%
  ...
  dplyr::mutate(LN__Pct_of_HLADRpos_Classical_Mo_minusDC3_Ki67pos = LN__Pct_of_HLADRpos_Classical_Mo_Ki67pos - LN__Pct_of_HLADRpos_DC3_Ki67pos,
...
#                 blood__Pct_of_HLADRpos_Classical_Mo_Ki67pos = NULL) 
# 
# curr_prep_colnames <- colnames(curr_prep)
# ### for each compartment
# for(curr_compartment in c("LN","blood")){
#   ### for each column
#   for(curr_col in curr_prep_colnames[grepl(paste0(curr_compartment,"__"),curr_prep_colnames)]){
#     curr_prep <- curr_prep %>%
#       ## rename HLADRpos
#       dplyr::mutate(denom = !!rlang::sym(paste0(curr_compartment,"__Pct_of_HLADRpos_HLA_DRpos_Ki67pos")),
#                     ### calculate proportion
#                     prop = 100*!!rlang::sym(curr_col)/denom)
#     ## rename prop
#     curr_prep[,paste0(curr_col,"__prop")] <- curr_prep[,"prop"]
#     ## remove prop
#     curr_prep <- curr_prep %>%
#       dplyr::select(-prop)
#   }
...
# 
# 
# curr_phen_long  <- curr_prep %>%
#   ### get only prop columns
#   dplyr::select(all_of(c("MacaqueID","Day",colnames(.)[grep("__prop",colnames(.))]))) %>%
#   ## remove HLA_DRpos totals (all 100)
#   dplyr::select(!all_of(c(colnames(.)[grepl("Pct_of_HLADRpos_HLA_DRpos_Ki67pos__prop",colnames(.))]))) %>%
#    ### get average for each day/compartment
#   # first make longer
#   tidyr::pivot_longer(cols = contains("__"), names_to = "comp_var", values_to = "value") %>%
#   dplyr::mutate(compartment = factor(str_before_first(comp_var,"__"), c("blood","LN")),
#                 #variable = str_after_first(comp_var,"__"),
#                 Day = factor(Day, c("D1","D3","D8","D22"))) %>%
#   # collect total 
#   group_by(compartment, Day, MacaqueID) %>%
#   dplyr::mutate(total_per_comp_time = sum(value)) %>%
...
#                 Day = factor(Day, c("D1","D3","D8","D22"))) %>%
#   # take mean across animals
#   dplyr::select(-comp_var) %>%
#   group_by(compartment, variable, Day) %>%
#   dplyr::summarise(mean = mean(value)) %>%
#   # add comp_time
#   dplyr::mutate(comp_time = paste0(compartment,"__",Day),
#                 variable = factor(str_before_last(variable,"__prop"), c("Pct_of_HLADRpos_pDC_Ki67pos",
#                                               "Pct_of_HLADRpos_cDC1_Ki67pos",
#                                               "Pct_of_HLADRpos_cDC2_Ki67pos",
#                                               "Pct_of_HLADRpos_Non_classical_Mo_Ki67pos",
#                                               "Pct_of_HLADRpos_Intermediate_Mo_Ki67pos",
...
### To name output files
contrast_name <- "FigS5B_mean_of_props"
...
```{r FigS5B_pies, fig.width=25, fig.height=26}
list_of_plots <- list()
...
for(i in unique(curr_phen_long$comp_time)){
 phen_to_plot <- curr_phen_long %>%
    ...
    ## add in labels
    dplyr::arrange(compartment,variable) %>%
    dplyr::mutate(label = paste0(round(mean,1),"%"),
 ...
 ### set colors
  color_df <- data.frame("variable"=unique(curr_phen_long$variable),
                         ...
                         "fill"=c("#D6D6D6","#0196FF","#73FCD7",
                                  "#FFFC00","#019051","#FF676D","#FF9301"),
                         "color"=c("grey50","#0533FF","#59A38E",
                                  "lightgoldenrod3","#019051","#FF676D","#FF9301"))
...
list_of_plots[[length(list_of_plots)+1]]<-g_legend(p1) ### grabs from the l
do.call("grid.arrange",c(list_of_plots, ncol=num_columns_for_plot))


list_of_plotted[[paste0(contrast_name)]] <-  curr_phen_long %>%
  dplyr::select(-comp_time)
```
...
curr_ds <- list_of_plotted[[paste0(contrast_name,"_log2FC")]] %>%
  ...
  dplyr::filter(variable %in% vars_for_selfCorr) %>%
  column_to_rownames("variable")
...
#### plotted is same as stats but trimmed
list_of_plotted[[paste0(CorrName)]] <- list_of_stats[[paste0(CorrName)]] %>%
  dplyr::select(-celltype1_vs_2) %>%
  ## remove diagonal
  dplyr::filter(celltype1 != celltype2) %>%
  ## remove inverse duplicates
 group_by(grp = paste(pmax(celltype1, celltype2), pmin(celltype1, celltype2), sep = "_")) %>%
...
```{r prepForDiffCounts_CellPops__Fig6F}
...
contrast_name <- "Fig6F_GC"

raw_or_adj <- "raw"
```

```{r diff_counts_calcs__Fig6F, ref.label=c('diff_counts_calcs'),  include = FALSE}
```
...
vars_for_AbCorr <-rev(c("LN__CD4_Tfh_CXCR3neg_Ki67pos",
                   ...
                   "LN__CD8_Tfh_CXCR3pos_Ki67pos",
                   "LN__Bcells_GC_BCL6pos_ki67pos_PctBCells"))
### load GC pops log2 data - wide with animal_time in columns, variables in row
contrast_name <- "Fig6G_GC"
curr_ds <- curr_df_long %>%
  ### trim to only the vars desired
  dplyr::filter(variable %in% vars_for_AbCorr) %>%
  ## make wider
  dplyr::mutate(animal_time = paste0(MacaqueID,"__",TimePoint),
                MacaqueID = NULL,
                TimePoint = NULL) %>%
  tidyr::pivot_wider(names_from = "animal_time", values_from = "log2_value") %>%
  column_to_rownames("variable")

list_of_plotted[[paste0(contrast_name,"_log2FC")]] <- curr_ds


### load Abs - wide with animal_time in rows, variables in columns
contrast_name <- "Fig6G_Abs"
curr_ds_abs <- data_Abs %>%
  ##rename sampleNAme
  dplyr::rename(Animal_TP=sampleName) %>%
  ### #### unify with cell pops
  dplyr::mutate(Animal_TP = str_replace_all(Animal_TP,"D8","D3"),
                Animal_TP = str_replace_all(Animal_TP,"D15","D8"),
                MacaqueID = NULL,
                Day = NULL) 
  

list_of_plotted[[paste0(contrast_name,"_log2FC")]] <- curr_ds_abs
...
```{r scatterplots_with_pvalues___FigS6A_prep}
contrast_name <- "Fig6BE"
curr_figure_panel <- "FigS6A"
...
list_of_stats[["FigS6AC"]] <- data.frame("FigS6A"="stats are in the Fig6BE tab")
```

```{r scatterplots_with_pvalues___FigS6A, ref.label=c('scatterplots_with_pvalues'), results='asis', fig.height=3, fig.width=10}
```
...
```{r lmerTest_byTime_FigS6B, ref.label=c('lmerTest_byTime'), include = FALSE}
```


```{r plotsPvalsByTime_oneZonePerPlot_LOGSCALE_FigS6B, ref.label=c('plotsPvalsByTime_oneZonePerPlot_LOGSCALE'), results='asis', fig.height=8, fig.width=8}
```
...
```{r scatterplots_with_pvalues___FigS6C_prep}
contrast_name <- "Fig6BE"
curr_figure_panel <- "FigS6C"
...
```{r scatterplots_with_pvalues___FigS6C, ref.label=c('scatterplots_with_pvalues'), results='asis', fig.height=7, fig.width=10}
```

## Supp Figure S7 {.tabset}

### FigS7 {.tabset}

Selected scatterplots

```{r corrScatterPlots_prep_FigS7}
########## Reload data from Fig 6F
### which dataset levels/FC, trimmed to the tab
curr_ds <- list_of_plotted[[paste0("Fig6F_GC_log2FC")]] %>%
  column_to_rownames("variable")
...
num_columns_for_plot <- 2
curr_figure_panel <- "FigS7"
```

```{r corrScatterplots_with_RandPvalues__FigS7, ref.label=c('corrScatterplots_with_RandPvalues'), results='asis', fig.height=10}
```
...
list_of_plotted[[paste0(curr_figure_panel)]] <- df_long %>%
  dplyr::select(-variable, -compartment) %>%
  dplyr::relocate(comp_var,MacaqueID,treat) %>%
  dplyr::rename(panel_variable=comp_var)
```
...
```{r setVars_Fig8E}
varcomp_to_plot_prep <- colnames(list_of_datasets[["AIM"]])[grepl("panel8E",colnames(list_of_datasets[["AIM"]]))]%>%
  str_remove_all("Fig8__")
## set order
varcomp_to_plot <- c(varcomp_to_plot_prep[grepl("TNF",varcomp_to_plot_prep)],
                     varcomp_to_plot_prep[grepl("IL2",varcomp_to_plot_prep)],
                     varcomp_to_plot_prep[grepl("CXCL11",varcomp_to_plot_prep)],
                     varcomp_to_plot_prep[grepl("CCL4",varcomp_to_plot_prep)],
                     varcomp_to_plot_prep[grepl("CXCL10",varcomp_to_plot_prep)],
                     varcomp_to_plot_prep[grepl("IFN",varcomp_to_plot_prep)])
                     

num_columns_for_plot <- 3
curr_figure_panel <- "Fig8E"
```
...
##save
saveRDS(list_of_plotted,file.path(outputRData_dir,paste0("list_of_plottedValues.xlsx")))
### openxlsx write.xlsx gives Error in x[is.na(x)] <- na.string : replacement has length zero
## try replacing NA with "na" to find where issue
# for(i in names(list_of_plotted)){
#   list_of_plotted[[i]][is.na(list_of_plotted[[i]])] <- 'na'
# }

openxlsx::write.xlsx(list_of_plotted,file.path(outputTABLES_dir,paste0("list_of_plottedValues.xlsx")))

#### add with button
xfun::embed_file(file.path(outputTABLES_dir,paste0("list_of_plottedValues.xlsx")), text = "Excel spreadsheet of plotted values")
