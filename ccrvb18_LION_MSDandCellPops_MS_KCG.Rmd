...
combo_df_long <- annotation_row %>%
 ...
 ####### plot
  ...
  cat(' \n \n') ### this is the key!
     ...
     list_of_plotted[[paste0("FigS1C_MSD_V4")]] <- combo_df_long
...
assay_arrangement <- c("FLT3L","CCL13.MCP.4","CCL22.MDC","CXCL12.SDF.1a",
                       "G.CSF","IFN.g","CCL19.MIP.3b") ### if trimming/ordering assays
...
```{r Lineplots_with_pvalues_FigS4A, ref.label=c('Lineplots_with_pvalues'), eval=TRUE, results='asis', fig.height=7, fig.width=8}
```
...
### load Abs
contrast_name <- "Fig6G_Abs"
curr_ds_abs <- list_of_plotted[[paste0(contrast_name,"_log2FC")]] %>%
  column_to_rownames("variable") %>%
  t() %>%
  data.frame() %>%
  #### remove rows with all NA
...
### which dataset levels/FC, trimmed to the tab
curr_ds <- list_of_plotted[[paste0("Fig6FG_GC_log2FC")]] %>%
  column_to_rownames("variable")
